    ...
    for(let j = 0; j < rows-1; j++){
      ...
      if(ready){
        ...
        if(whichAI == 'ML5'){
          ...
          await nn.classify(input, GuessMl5Model);

        }
        else {
          let input = tf.tensor2d([level[i][j], level[i+1][j], level[i+1][j+1], level[i][j+1]], [1 , 4]);
          guessedCase = await nn2.predict(input).argMax(-1).dataSync()[0];
          await sleep(2);
        }
