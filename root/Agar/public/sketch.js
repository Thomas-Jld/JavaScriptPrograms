...
function setup() {
  pixelDensity(1);
  Hei = window.innerHeight*0.97;
  Wid = window.innerWidth*0.97;
  // Hei = 900;
  // Wid = 900;
  console.log(Wid + "*" + Hei);
  ...
  console.log(socket);
  //Création du blob
  blob = new Blob(random(size)-size/2, random(size)-size/2, 17);
...
//Manger ou être mangé
  ...
  for(var j = 0; j<blobs.length; j++){
    ...
    if (d1 < blob.r && blob.r > blobs[j].r && blobs[j].id !== socket.id) {
    ...
    if (d1 < blobs[j].r && blob.r < blobs[j].r && blobs[j].id !== socket.id) {
      blob.r = 0;
      alive = false;
    }
  }
